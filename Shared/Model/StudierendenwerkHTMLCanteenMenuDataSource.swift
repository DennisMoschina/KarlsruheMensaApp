//
//  StudierendenwerkHTMLCanteenMenuDataSource.swift
//  Mensa
//
...
final class StudierendenwerkHTMLCanteenMenuDataSource: CanteenMenuDataSource {
    private var imageCache: [String: URL] = [:]
    private let imageCacheQueue = DispatchQueue(label: "StudierendenwerkHTMLCanteenMenuDataSource.imageCache")
    ...
    /// Creates an HTML source with an empty in-memory image cache.
    init() {}
    ...
    private func enrichFoodMapWithMealDetails(
        ...
        let imageAssignmentQueue = DispatchQueue(label: "StudierendenwerkHTMLCanteenMenuDataSource.imageAssignment")
...
"""
    ...
    private func buildFetchChunks(for requestedDays: [CanteenMenuDayRequest]) -> [FetchChunk] {
        let calendar = Calendar.current
